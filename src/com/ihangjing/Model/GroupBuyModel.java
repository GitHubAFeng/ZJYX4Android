...
public class GroupBuyModel {// 买搭增
	...
	public List<GroupBuyAttrModel> listSpec;// 规格
	...
	public void JsonToBean(JSONObject localJSONObject, int sr) {
		...
		try {
			...
			togoName = localJSONObject.getString("shopname");
			
			
			

			listSpec = new ArrayList<GroupBuyAttrModel>();
			GroupBuyAttrModel mode = new GroupBuyAttrModel();
			mode.cId = 0;
			mode.name = getFoodName();//赠品名称
			mode.price = this.price;//赠品价格
			listSpec.add(mode);
			
			startTime = localJSONObject.getString("stardate");
			
			endTime = localJSONObject.getString("enddate");
			...
			if (sr == 1) {
				...
				notice = localJSONObject.getString("notice");
